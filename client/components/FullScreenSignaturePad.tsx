...
export function FullScreenSignaturePad({
  ...
  useEffect(() => {
    ...
    // Set canvas to a fixed size that scales properly across devices
    const isMobile = window.innerWidth < 768;
    const canvasWidth = isMobile ? 800 : 1200; // Fixed canvas resolution for consistent output
    const canvasHeight = isMobile ? 400 : 600;

    // Set canvas to fixed size for consistent output
    const resize = () => {
    ...
    const setupCanvas = () => {
      ...
      ctx.lineWidth = isMobile ? 4 : 3; // Slightly thicker on mobile
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
    ...
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
      ...
      <div className="flex-1 relative bg-gray-50">
        ...
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-70 text-white px-4 py-2 rounded-lg text-center">
          <p className="text-lg font-medium">Client Signature Required</p>
